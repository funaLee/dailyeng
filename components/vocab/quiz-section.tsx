...
export function QuizSection({ items, topicId }: QuizSectionProps) {
  ...
  const handleSubmit = () => {
    ...
    if (isCorrect) {
      ...
      setWrongItems((prev) => [...prev, { item: current, userAnswer: selectedAnswer }])
    }
  }
        ...
        <Card className="p-8 text-center space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="p-4 bg-red-500/10 rounded-lg">
              <p className="text-3xl font-bold text-red-500">{items.length - score}</p>
              <p className="text-sm text-muted-foreground mt-1">Incorrect</p>
            </div>
            <div className="p-4 bg-yellow-500/10 rounded-lg">
              <p className="text-3xl font-bold text-yellow-500">{percentage}%</p>
          ...
          <div className="p-4 bg-green-500/10 rounded-lg border border-green-500/20">
            <div className="flex items-center justify-center gap-2 mb-2">
              ...
              <p className="font-semibold text-green-700">+{xpEarned} XP Earned!</p>
            </div>
          ...
          {wrongItems.length > 0 && (
            <div className="space-y-3">
              <p className="text-sm font-medium">You got {wrongItems.length} items wrong</p>
              <Button onClick={handleAddToFlashcards} variant="outline" className="w-full gap-2 bg-transparent">
                ...
                <div key={idx} className="p-4 border border-red-200 rounded-lg bg-red-50">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div>
                      ...
                      <p className="text-green-600 font-medium">{item.item.correctAnswer}</p>
                    </div>
                    <div className="pt-2 border-t border-red-200">
                      <p className="text-muted-foreground text-xs">{item.item.explanation}</p>
                    </div>
                  </div>
                </div>
              ))}
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        ...
        {current.type === "matching" && (
          <div className="space-y-3 mb-6">
            <p className="text-sm text-muted-foreground mb-4">Match the items:</p>
            {current.options.map((option, idx) => (
              <button
                key={idx}
                onClick={() => !showResult && setSelectedAnswer(option)}
        ...
        <div className="flex gap-3 mt-6">
          {!showResult ? (
            ...
            <Button onClick={handleNext} className="flex-1">
              {currentIndex < items.length - 1 ? "Next Question" : "Finish Quiz"}
            </Button>
          )}
        </div>
      </Card>
