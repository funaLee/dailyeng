...
import { Maximize2, Minimize2, ZoomIn, ZoomOut, RotateCcw, X, Volume2, BookOpen, ChevronRight, ChevronDown, Sparkles } from "lucide-react"
...
export function VocabMindmap({ topicGroups }: VocabMindmapProps) {
  ...
  const layoutData = useMemo(() => {
    ...
    topicGroups.forEach((group, i) => {
      ...
      const topicStartAngle = baseAngle - (topicCount > 1 ? topicSpread / 2 : 0)
      ...
      topics.forEach((topic, j) => {
        ...
        if (isExpanded) {
          ...
          if (wordCount > 0) {
            ...
            const dynamicRadius = R_WORD_BASE + (wordCount > 8 ? (wordCount - 8) * 12 : 0)
            const wordStep = wordCount > 1 ? dynamicSpread / (wordCount - 1) : 0
            const wordStartAngle = currentTopicAngle - (wordCount > 1 ? dynamicSpread / 2 : 0)
  ...
  const handleWordClick = useCallback((word: VocabItem, e: React.MouseEvent) => {
    e.stopPropagation()
  ...
  const containerClasses = isFullscreen ? "fixed inset-0 z-50 bg-background h-screen w-screen" : "relative w-full h-[700px] bg-background/50"

  return (
    <div className={`${containerClasses} flex flex-col font-sans text-foreground rounded-xl overflow-hidden border border-border`} ref={containerRef}>
      ...
      <div className="absolute top-6 right-6 z-20 flex items-center gap-1.5 p-1.5 bg-background/80 backdrop-blur-md border border-border/60 rounded-full shadow-lg shadow-primary-900/5">
        <Button variant="ghost" size="icon" onClick={() => setScale(s => Math.max(s - 0.2, 0.2))} className="h-9 w-9 rounded-full hover:bg-muted text-muted-foreground"><ZoomOut className="h-4 w-4" /></Button>
        <span className="text-xs font-bold w-12 text-center text-foreground tabular-nums">{Math.round(scale * 100)}%</span>
        <Button variant="ghost" size="icon" onClick={() => setScale(s => Math.min(s + 0.2, 4))} className="h-9 w-9 rounded-full hover:bg-muted text-muted-foreground"><ZoomIn className="h-4 w-4" /></Button>
        <div className="w-px h-5 bg-border mx-1" />
        <Button variant="ghost" size="icon" onClick={() => { setScale(0.9); setPosition({x:0, y:0}) }} className="h-9 w-9 rounded-full hover:bg-muted text-muted-foreground"><RotateCcw className="h-4 w-4" /></Button>
        <Button variant="ghost" size="icon" onClick={() => setIsFullscreen(!isFullscreen)} className="h-9 w-9 rounded-full hover:bg-muted text-muted-foreground">
      ...
      <div 
        ...
        style={{ 
          backgroundImage: 'radial-gradient(var(--gray-300) 1.5px, transparent 1.5px)', 
        ...
        >
          ...
          <svg className="overflow-visible pointer-events-none absolute left-0 top-0">
            ...
            {layoutData.map((node) => {
              ...
              return (
                <path
                  key={`edge-${node.id}`}
                  d={`M ${node.parent.x} ${node.parent.y} C ${node.parent.x} ${node.parent.y}, ${(node.x + node.parent.x)/2} ${(node.y + node.parent.y)/2}, ${node.x} ${node.y}`}
                  stroke={node.style.line} // CSS Variable used here
                  strokeWidth={node.type === "group" ? 4 : node.type === "topic" ? 3 : 1.5}
          ...
          {layoutData.map((node) => {
            ...
            if (node.type === "center") {
              return (
                ...
                >
                  ...
                  <span className={`font-black text-2xl tracking-tight ${node.style.text}`}>{node.data.name}</span>
                  <span className="text-gray-400 text-xs font-medium mt-1 uppercase tracking-widest">Mindmap</span>
                </div>
              )
            }
            ...
            if (node.type === "group") {
              return (
                <div key={node.id}
                  ...
                  style={{ left: node.x, top: node.y }}
                >
                  <span className={`font-extrabold text-xl tracking-tight ${node.style.text}`}>{node.data.name}</span>
                </div>
              )
            }
            ...
            if (node.type === "topic") {
              ...
              return (
                <div key={node.id}
                  className={`interactive-area absolute transform -translate-x-1/2 -translate-y-1/2 z-20 transition-all duration-500 ease-out ${isExpanded ? 'scale-110 z-30' : 'hover:scale-105'}`}
                  style={{ left: node.x, top: node.y }}
                ...
                >
                  <div className={`
                    ...
                    ${isExpanded ? `bg-background ring-4 ring-offset-0 ring-background/50 ${node.style.border}` : 'bg-background/90 hover:bg-background border-transparent'}
                  `}>
                    <span className={`font-bold text-lg ${node.style.text}`}>{node.data.title}</span>
                    
                    <div className={`flex items-center justify-center h-6 min-w-[24px] px-2 rounded-full text-xs font-bold transition-colors ${isExpanded ? 'bg-foreground text-background' : `${node.style.badgeBg} ${node.style.badgeText}`}`}>
                      {wordCount}
                    </div>
                ...
                >
                  <div className="group relative transition-all duration-300 hover:scale-110 hover:-translate-y-1">
                     <div className={`
                        ...
                        ${node.style.border}
                     `}>
                        <div className="w-2 h-2 rounded-full" style={{backgroundColor: node.style.line}}></div>
                        <span className="text-sm font-bold text-foreground whitespace-nowrap">{node.data.word}</span>
                     </div>
                  </div>
                </div>
              )
      ...
      {selectedWord && (
        <div
          className={`absolute z-50 w-72 animate-in fade-in duration-200 ease-out
            ${popupPlacement === 'top' ? 'slide-in-from-bottom-2' : 'slide-in-from-top-2'}
          `}
          style={{ 
            left: popupPosition.x, 
            top: popupPosition.y, 
            transform: `translate(-50%, ${popupPlacement === 'top' ? '-100%' : '0'}) translateY(${popupPlacement === 'top' ? '-16px' : '36px'})`
          }}
          ...
          <Card className="overflow-hidden rounded-xl border border-border shadow-2xl shadow-primary-900/10 bg-background/95 backdrop-blur-xl">
             <div className="p-3">
                <div className="flex items-start justify-between mb-1">
                  <div>
                    <h3 className="text-lg font-bold text-foreground tracking-tight">{selectedWord.word}</h3>
                    <p className="text-xs text-muted-foreground font-mono">{selectedWord.pronunciation}</p>
                  </div>
                  <Button variant="ghost" size="icon" className="h-6 w-6 -mr-1 -mt-1 rounded-full hover:bg-muted text-muted-foreground" onClick={() => setSelectedWord(null)}>
                ...
                <div className="space-y-1.5 mb-3">
                    <p className="text-sm text-foreground leading-snug">{selectedWord.meaning}</p>
                    {selectedWord.vietnameseMeaning && (
                      <p className="text-sm text-muted-foreground italic border-l-2 border-primary-100 pl-2">{selectedWord.vietnameseMeaning}</p>
                    )}
                </div>

                {selectedWord.exampleSentence && (
                  <div className="p-2 bg-muted/50 rounded-lg border border-border mb-3">
                    <p className="text-xs text-muted-foreground italic">"{selectedWord.exampleSentence}"</p>
                  </div>
                )}

                <div className="flex gap-2">
                    <Button size="sm" variant="outline" className="h-8 text-xs flex-1 bg-background hover:bg-muted">
                      <Volume2 className="h-3 w-3 mr-1.5 text-primary-600"/> Listen
                    </Button>
                    <Button size="sm" className="h-8 text-xs flex-1 bg-primary-600 hover:bg-primary-700 text-white shadow-sm">
             ...
             <div 
               className={`absolute left-1/2 w-3 h-3 bg-background border-r border-b border-border transform rotate-45 -translate-x-1/2
                 ${popupPlacement === 'top' ? 'bottom-0 translate-y-1/2' : 'top-0 -translate-y-1/2 border-l border-t border-r-0 border-b-0'}
               `}
             ></div>
          </Card>
        </div>
