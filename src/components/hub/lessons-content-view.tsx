...
export function LessonsContentView({
  subTopicTitle,
  lessonGroups,
  onLessonClick,
}: LessonsContentViewProps) {
    ...
    <Card className="rounded-3xl border-[1.4px] border-primary-200 bg-card overflow-hidden shadow-lg">
      ...
      <div className="p-6 space-y-6">
        {lessonGroups.map((group) => {
          const config =
            lessonTypeConfig[group.type] || lessonTypeConfig.vocabulary;
          const Icon = config.icon;

          return (
            <div
              key={group.id}
              className="rounded-2xl border-[1.4px] border-primary-200 overflow-hidden"
            >
              ...
              <div className="divide-y divide-border">
                {group.lessons.map((lesson) => {
                  const isFirstNotStarted =
                    lesson.id === globalFirstNotStartedId;
