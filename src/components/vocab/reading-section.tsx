...
export function ReadingSection({ passage }: ReadingSectionProps) {
  ...
  const glossaryMap = new Map(
    passage.glossary.map((item) => [item.word.toLowerCase(), item])
  );
  ...
  const handleToggleHint = () => {
    ...
    if (!showHint) {
      const questionWords = currentQ?.question
        .toLowerCase()
        .split(" ")
        .filter((w) => w.length > 4);
  ...
  const handleCheck = () => {
    if (currentQ) {
      ...
      if (answers[currentQ.id] !== currentQ.correctAnswer) {
        const correctWords = currentQ.correctAnswer
          .toLowerCase()
          .split(" ")
          .filter((w) => w.length > 4);
        const content = passage.content.toLowerCase();
        ...
        <Card className="p-8 text-center space-y-6">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Great job on finishing the reading practice
            </p>
          </div>
