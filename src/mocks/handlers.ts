...
export const handlers = [
  ...
  http.post("/api/quizzes/submit", async ({ request }) => {
    const { topicId, answers } = (await request.json()) as any;
  ...
  http.post("/api/srs/review", async ({ request }) => {
    const { cardId, quality } = (await request.json()) as any;
    return HttpResponse.json({
  ...
  http.post("/api/ai/translate", async ({ request }) => {
    const { text, targetLang } = (await request.json()) as any;
    return HttpResponse.json({
  ...
  http.post("/api/ai/feedback", async ({ request }) => {
    const { text, type } = (await request.json()) as any;
    return HttpResponse.json({
      pronunciation:
