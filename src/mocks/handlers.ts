...
interface SRSReviewBody {
  cardId: string;
  quality: number;
}

interface PlanSetupBody {
  [key: string]: any;
}
...
export const handlers = [
  ...
  http.post("/api/topics", async ({ request }) => {
    const data = (await request.json()) as TopicBody;
  ...
  http.post("/api/quizzes/submit", async ({ request }) => {
    const { topicId, answers } = (await request.json()) as QuizSubmitBody;
  ...
  http.post("/api/ai/create-topic", async ({ request }) => {
    const { prompt } = (await request.json()) as CreateTopicBody;
    return HttpResponse.json(
      {
  ...
  http.post("/api/speaking/submit-turn", async ({ request }) => {
    const { sessionId, text } = (await request.json()) as SubmitTurnBody;
    const responses = [
  ...
  http.post("/api/flashcards", async ({ request }) => {
    const data = (await request.json()) as FlashcardBody;
    return HttpResponse.json(
      { ...data, id: Date.now().toString(), createdAt: new Date() },
      { status: 201 }
    );
  }),
  ...
  http.post("/api/srs/review", async ({ request }) => {
    const { cardId, quality } = (await request.json()) as SRSReviewBody;
    return HttpResponse.json({
  ...
  http.post("/api/plan/setup", async ({ request }) => {
    const data = (await request.json()) as PlanSetupBody;
    return HttpResponse.json(
      { ...data, id: Date.now().toString(), createdAt: new Date() },
      { status: 201 }
    );
  ...
  http.post("/api/ai/translate", async ({ request }) => {
    const { text, targetLang } = (await request.json()) as TranslateBody;
    return HttpResponse.json({
  ...
  http.post("/api/ai/feedback", async ({ request }) => {
    const { text, type } = (await request.json()) as FeedbackBody;
    return HttpResponse.json({
      pronunciation:
