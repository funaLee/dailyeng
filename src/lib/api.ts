...
import type { Topic, VocabItem, QuizItem, Flashcard, SpeakingScenario, StudyPlan, ProfileStats } from "@/types"
...
export const speakingApi = {
  ...
  async createScenario(data: Omit<SpeakingScenario, "id">): Promise<SpeakingScenario> {
    await new Promise((resolve) => setTimeout(resolve, 500))
    return {
      ...data,
      id: Date.now().toString(),
  ...
  async submitTurn(sessionId: string, text: string): Promise<{ response: string; scores: Record<string, number> }> {
